...
class MesTestsSemaines(unittest.TestCase):
    annee = 0
    le_num_semaine = 0
    maria = ma_connect()
    lundi = ""
    ...
    def test_01_semaine(self):
        mon_lundi = self.utilitaire_prem_jour_sem(6)
        self.assertNotEqual(mon_lundi,None)
        print("LUNDI test %s" % mon_lundi)

    def test_02_renseigne_jours_semaine(self):
        ...
        for j in les_jours:
            incr = incr + 1
            les_jours[incr][1] = (self.utilitaire_prem_jour_sem(6) + timedelta(days=incr)).strftime('%Y-%m-%d')
        print("\n" + str(les_jours))
